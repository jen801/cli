...
t.test('config delete single key', async t => {
  ...
  t.not(contents.includes('access='), 'access was removed on disk')
})
...
t.test('config delete multiple keys', async t => {
  ...
  const contents = await fs.readFile(join(home, '.npmrc'), { encoding: 'utf8' })
  t.not(contents.includes('access='), 'access was removed on disk')
  t.not(contents.includes('all='), 'all was removed on disk')
})
...
t.test('config set key value', async t => {
  ...
  const contents = await fs.readFile(join(home, '.npmrc'), { encoding: 'utf8' })
  t.ok(contents.includes('access=restricted'), 'wrote access to disk')
})
...
t.test('config set key1 value1 key2=value2 key3', async t => {
  ...
  const contents = await fs.readFile(join(home, '.npmrc'), { encoding: 'utf8' })
  t.ok(contents.includes('access=restricted'), 'access was written to disk')
  t.ok(contents.includes('all=false'), 'all was written to disk')
  t.ok(contents.includes('audit=false'), 'audit was written to disk')
})
